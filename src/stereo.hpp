	...
	int init(HWND hwnd, int width, int height)
	{
...
#if defined(USE_OVR)
		{
			...
			for (int i = 0; i < offscreen_buffers_ * 2; i ++) {
				...
				uniq_.dev()->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &eyetex, D3D12_RESOURCE_STATE_RENDER_TARGET, &cv, IID_PPV_ARGS(&ovr_rtv[i]));
...
protected:
	...
	void on_draw()
	{
		...
		try {
			...
			hr = pre_cmd_list_->Reset(pre_cmd_alloc_[idx].Get(), pso);
...
#if defined(DUMMY_OVR)
			ID3D12Resource* p[] = {rt_.rt(idx), eye_.rt(idx) };
#else
...
#if defined(DUMMY_OVR)
					cmd->OMSetRenderTargets(1, &rtv0, FALSE, &dsv);
					cmd->ClearRenderTargetView(rtv0, col3, 0, nullptr);
#else
					D3D12_RESOURCE_BARRIER barrier_begin = {
						D3D12_RESOURCE_BARRIER_TYPE_TRANSITION, D3D12_RESOURCE_BARRIER_FLAG_NONE,
						{ 	eye_.rt(idx),
			...
			finishcmd->ResourceBarrier(1, &barrier_end);
#endif
			finishcmd->Close();
...
#else
			/* left only */
			ID3D12CommandList* l[] = {stereo_cmd_list_[0].Get(), finish_cmd_list_.Get()};
			uniq_.queue()->ExecuteCommandLists(std::extent< decltype(l) >::value, l);
#endif
